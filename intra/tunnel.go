...
func NewTunnel(fakedns string, dohdns doh.Transport, link stack.LinkEndpoint, dialer *net.Dialer, config *net.ListenConfig, listener Listener) (Tunnel, error) {
	tcp, udp, err := getConnectionHandlers(fakedns, dialer, config, listener)
	if err != nil {
		return nil, err
	}
	base, err := tunnel.NewTunnel(link, tcp, udp)
	if err != nil {
		return nil, err
	}
	t := &intratunnel{
